...
def generate_weekly_schedule(faculty, year, chronotype, start_hour=8, end_hour=20):
    """Generate a complete weekly schedule for a student."""
    schedule = {}
    
    for day in range(1, 6):  # Monday to Friday
        # Generate daily lecture schedule
        daily_schedule = generate_lectures(adjust_by_year(faculty_characteristics[faculty], year))
        
        # Add library visits
        daily_schedule = generate_library_visits(
            adjust_by_year(faculty_characteristics[faculty], year),
            daily_schedule,
            chronotype,
            start_hour,
            end_hour
        ...
        # Add entries to weekly schedule
        for hour, activity in daily_schedule.items():
...
def generate_student_population(num_students, faculty_distribution=None, start_hour=8, end_hour=20):
    ...
    for i in range(num_students):
        ...
        # 5. Generate schedule
        schedule = generate_weekly_schedule(faculty, year, chronotype, start_hour, end_hour)
